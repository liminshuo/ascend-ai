...
> Stay up to date with the latest Mintlify product updates, new features, platform improvements, bug fixes, and documentation tooling changes.
...
<Update label="July 24, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"GraphQL and SDK references, analytics streaming, agent integrations, and more" }}>
  ...
  * **GraphQL API reference pages:** Generate reference pages from a GraphQL SDL schema, with linked types and example queries, mutations, and responses. See [GraphQL setup](/docs/api-playground/graphql-setup).
  * **SDK reference generation:** Build SDK reference pages directly from the output of TypeDoc, DocFX, Javadoc, Sphinx, or phpDocumentor. See [SDK reference setup](/docs/api-playground/sdk-reference-setup).
  ...
  * **More ways to collaborate:** Start [comment threads](/docs/editor/comments) on an entire page, mention yourself to leave a reminder, and use keybaord shortcuts <kbd>Option</kbd> + <kbd>C</kbd> or <kbd>Option</kbd> + <kbd>S</kbd> to comment on or suggest changes to selected text.
  ...
  * **Shared and personal integrations:** Give the Slack agent and automations read-only context from shared tools, or connect a personal account for the agent to use when you chat with it. See [integrations](/docs/automations/integrations).
  * **Integration triggers:** On Enterprise plans, trigger automations from third-party events such as a support ticket update.
  * **Slack agent reads PR activity:** The [Slack agent](/docs/agent/slack) now reads PR comments, reviews, and CI checks, and follows up in the thread after reverting a change based on feedback.

  ## Analytics and data streaming

  * **Analytics data warehouse streaming:** On Enterprise plans, stream analytics events to Amazon S3 in near real time, with controls for which event categories and deployments to include. See [analytics streaming](/docs/optimize/analytics#stream-analytics-events).
  * **Audit log streaming:** On Enterprise plans, stream audit events to Amazon S3 or Datadog for security monitoring and long-term retention. See [audit logs](/docs/dashboard/audit-logs#stream-audit-logs).
  * **Automation analytics dashboard:** A new analytics view for [automations](/docs/automations) with metrics on pull requests and credit use.
  ...
  * **Deployment setup:** Use `mint add-domain` to configure a [custom domain](/docs/customize/custom-domain) from the CLI.
  * **`mint signup`:** Create a Mintlify account from the terminal, including agent-driven CLI workflows. See [CLI commands](/docs/cli/commands#mint-signup).
  * **Publishing controls:** Share a [changelog](/docs/changelog) filtered to a specific tag, and ignore `.mintlify` during deploys by default except for `skills` and `specs`.
  * **Simpler file trees:** Write [`FileTree`](/docs/components/tree) components as nested Markdown lists to document folder structures with less markup.
...
<Update label="July 16, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"Page improvements, new analytics dashboard, and more" }}>
  ...
  * **Related pages:** Display [related pages](/docs/organize/related-pages) to help readers discover additional relevant content.
  * **Per-page contextual menus:** Configure the [contextual menu](/docs/ai/contextual-menu) per page in frontmatter to override the site-wide options.
  * **Mermaid fullscreen mode:** Readers can expand [Mermaid diagrams](/docs/components/mermaid-diagrams) into a fullscreen view for easier inspection of complex charts.
  ...
  * **Drafts:** Work on unpublished changes without creating a branch or using any Git commands. See [Drafts](/docs/editor/drafts) for more information.
  * **Lock the deploy branch:** The editor now prompts you to lock your deploy branch if it detects branch protections.
  * **Faster [editor agent](/docs/editor) with better undo/redo:** Agent updates now stream in instantly, navigation is faster, and content and navigation-tree changes are undoable and redoable.
  * **Collapsible heading state persists per user:** Your expanded and collapsed section preferences carry across editor sessions.
  * **Visibility component in the editor:** The [visual editor](/docs/editor) now supports the [`Visibility`](/docs/components/visibility) component so you can author separate content for humans and AI agents without leaving the editor.
  ...
  * **Trailing-slash SEO config:** New `seo.trailingSlash` option in [`docs.json`](/docs/organize/settings-seo) enforces consistent trailing-slash URLs across your site.

  ## Automations and Slack agent

  * **Agent configuration moved into Automations:** Agent settings are now on the [Automations](/docs/automations) page.
...
<Update label="July 10, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"Custom subpath hosting, editor comments and suggestions, search filters, and more" }}>
  ## Custom domain, subdomain, and subpath hosting

  * **Improved setup for custom domain, subdomain, and subpath hosting:** Updated UI and steps on the [Domain setup](https://app.mintlify.com/settings/deployment/custom-domain) page. See [Custom domain](/docs/customize/custom-domain) and [Host at a subpath](/docs/deploy/docs-subpath) for more information.

  ## Editor comments, suggestions, and performance

  * **Comments and suggestions:** Leave threaded comments on any text in the [editor](/docs/editor), reply, resolve, and @mention teammates with email notifications. Suggestion mode lets reviewers propose edits for authors to accept or reject.
  ...
  * **Search filters:** Add [filters to your docs search](/docs/optimize/search) so readers can narrow results by product, version, or other categories.
  ...
  * **Require SSO with break-glass access:** Enforce SSO sign-in for your entire organization, with designated break-glass members who can still sign in directly if your identity provider is unavailable. See [SSO](/docs/dashboard/sso).
  * **Network access IP allowlists:** Restrict dashboard access to trusted IP ranges with the new [network access](/docs/dashboard/network-access) settings, and apply IP allowlists to external API keys for granular control.
  * **Dashboard session lifetimes and revocation:** Configure how long dashboard sessions stay valid and revoke individual sessions from the members page, complementing the existing [SSO controls](/docs/dashboard/sso).
  ...
  * **Clarifying questions:** The [assistant](/docs/assistant) can now ask clarifying questions when a user's request is ambiguous. The assistant presents the questions as selectable option cards in chat.
  * **Update from support feedback automation:** The new [Update from support feedback automation](/docs/automations) reviews resolved support conversations from connected third-party apps and drafts documentation updates.
  * **Contact form opt-out:** Turn off the assistant's in-chat contact form from [deflection settings](/docs/assistant/configure) if you don't want the assistant to hand off to support.
  * **Slack agent improvements:** The [Slack agent](/docs/agent/slack) respects deploy-branch protection when choosing its review process, hides its review-process dropdown for Mintlify-hosted deployments, retries file fetches on upload races, and steers feedback reads to code mode.
  ...
  * **Granular PDF exports:** [Export PDFs](/docs/optimize/pdf-exports) for specific pages or locales instead of the entire site.
  ...
  * **Custom OAuth redirect URIs for MCP:** Register your own allowed OAuth redirect URIs for your [MCP server](/docs/ai/model-context-protocol) from the dashboard.
  * **API playground:** Paste a JSON body into the [playground](/docs/api-playground/overview) to auto-fill request body fields.
...
<Update label="July 2, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"Assistant custom skills, SCIM provisioning, and more" }}>
  ## Assistant skills and support handoff

  * **Assistant skills:** Add [skills](/docs/assistant/skills) that the assistant loads on demand via a `loadSkill` tool call, so it can answer with your team's hand-written, opinionated guidance instead of relying on search results alone.
  * **Assistant contact form:** The [assistant](/docs/assistant) can now present an in-chat contact form to hand off to support or sales when a question can't be answered from your content.
  * **Themed query categories in assistant analytics:** [Assistant analytics](/docs/optimize/analytics) groups related questions into categories and shows thumbs-up/thumbs-down sentiment per category, so you can review frequent topics your users are asking about.
  * **Hosted MCP submit feedback tool:** Agents connected to your [hosted search MCP server](/docs/ai/model-context-protocol#mcp-tools) can now report issues back to your team, recorded as unhelpful feedback in your analytics dashboard.

  ## Enterprise security

  * **Session IP allowlist:** Restrict dashboard sessions to allowed IP ranges as part of your [SSO and session security policy](/docs/dashboard/sso).
  ...
  * **Admin MCP consent UI:** Redesigned [admin MCP](/docs/ai/model-context-protocol) consent screen with a multi-select deployment picker, per-deployment scopes, and a new `list_deployments` discovery tool. Consent, auth paths, and code-mode SDK client are now scoped to the selected organization and deployment.
  * **Hosted MCP admin connect page:** New admin connect page and hosted MCP auth settings in the dashboard under Settings.
  * **`searchable` frontmatter field:** Set `searchable: false` on a [page](/docs/organize/pages) to omit it from site search results.
  ...
  * **Publish concurrency safety in the editor:** The [editor](/docs/editor) broadcasts when a publish starts so other clients block immediately, and the publish dropdown can be dismissed during loading. Publish failures now include a summary.
  * **Resumable editor agent streams:** The [editor agent](/docs/editor) reconnects to in-flight streams after a disconnect instead of restarting from scratch.
  * **Slack agent resilience:** The [Slack agent](/docs/agent/slack) recovers cleanly when a message is interrupted and no longer leaves loaders spinning; switch-deployment flows are hardened.
  * **SSO copy refresh:** The [SSO](/docs/dashboard/sso) settings page has clearer sections and updated GitLab configuration instructions.
  * **Usage activity CSV export:** [Export your usage activity log](/docs/credits#export-usage-activity) to CSV from the Usage page for deeper analysis, reporting, or record-keeping.

  ## Bug fixes

  * Multi-repo deployments no longer prepend the mount path to OpenAPI endpoint entries.
  * `llms.txt` OpenAPI spec URLs no longer double up the custom base path.
  * Three-way merges in the editor no longer duplicate `Step`, `iframe`, or static HTML blocks.
  * The SEO audit no longer overwrites a page's `sidebarTitle`.
  ...
  * Automation run rows on the [automations page](/docs/automations) are clickable to their details page, and the weekly run comparison indicator was removed.
...
<Update label="June 26, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"Google Drive agent integration, multi-source git settings, new onboarding, and more" }}>
  ...
  * **Google Drive agent integration:** The agent can now pull context from Google Drive alongside Slack, Notion, Linear, Jira, and other [agent integrations](/docs/agent/integrations).
  * **Multi-repository deployments:** Configure and reorder multiple Git sources for a deployment from the dashboard. See [multi-repo setup](/docs/deploy/multi-repo).
  * **Assistant page navigation:** The [AI assistant](/docs/assistant) can navigate users directly to a relevant page as part of its answer.
  * **Custom agent instructions in `llms.txt` and markdown:** Custom agent instructions are now included in [`llms.txt`, `llms-full.txt`](/docs/ai/llmstxt), and per-page generated markdown.
  * **Mintlify Slack app in the Slack Marketplace:** The [Mintlify Slack app](/docs/agent/slack) is now publicly listed and installable from the Slack Marketplace.

  ## Improvements

  * **Automation PR grouping:** When an [automation](/docs/automations) run proposes new changes, it now checks whether they should be appended to an existing open docs PR rather than creating a new one, reducing duplicate PRs and review burden. Applies to Maintenance and Update from Code Changes automations.
  * **Automation GitHub PR review experience:** Docs PRs created by [automations](/docs/automations) now post a comment each time they're updated, explaining what changed and which code PRs were used as context. PR titles identify which automation created them, reviewers are auto-assigned based on who changed the relevant code, and follow-up runs update the PR summary in place.
  * **HTML alternate link for markdown pages:** Every page now exposes an HTML `<link rel="alternate">` pointing to its [markdown version](/docs/ai/llmstxt), making it easier for AI tools and crawlers to discover the LLM-friendly content.
  * **Real-time publish status in the editor:** The [editor](/docs/editor) now shows publish actions as they happen so you can see exactly when a change goes live.
  ...
  * **SSO enforcement on the frontend:** The dashboard now honors backend 403s for [SSO-only orgs](/docs/dashboard/sso) and routes verified-domain users through SSO during sign-in.
  ...
  * **MCP server identity and instructions:** The [Mintlify MCP server](/docs/ai/mintlify-mcp) now advertises instructions on initialize, helping clients use it more effectively.
...
<Update label="June 19, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"Automations rename, SSO enforcement, self-hosting upgrades, and more" }}>
  ...
  * **Workflows are now automations:** Introducing [automations](/docs/automations). They have the same functionality as workflows, just a new name to better describe what they're helping you do.
  * **SSO enforcement and breakglass:** Org owners can enforce [SSO-only sign-in](/docs/dashboard/sso) for members, configure verified domains, and manage breakglass emails from the dashboard for emergency access.
  * **Full PDF export for authenticated sites:** [Authenticated sites](/docs/deploy/authentication-setup) can now generate full PDF exports, with auth cookies parsed so images and assets are included.
  ...
  * **Editor view changes:** Open an automation run's changed pages directly in the [editor](/docs/editor) from the run review UI or Slack. Changed pages are highlighted, and unlisted pages are openable from the branch diff.
...
<Update label="June 12, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"Authenticated sites on Starter plan, editor improvements, and more" }}>
  ## Authenticated sites on Starter plan

  * **Mintlify auth on Starter:** [Mintlify-managed authentication](/docs/deploy/authentication-setup) is now available on the Starter plan.

  ## Editor improvements

  * **Drafts and publishing flow:** Make changes in a draft, request review, and publish — all from the [web editor](/docs/editor). Drafts autocommit as you work, and renaming or updating a draft updates the underlying pull request.
  ...
  * **Render reason in agent panel:** The editor's agent panel now shows the reason behind each action it takes.
  * **PR review polling:** The editor polls for PR review status so reviewer feedback shows up without a refresh.
  ...
  * Duplicate navigation group names render without collisions.
  * Renamed contextual menu action "Open in Windsurf" to "Open in Devin Desktop" and uses the `devin://` deep link.
</Update>

<Update label="June 5, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"Agent and automation integrations, search upgrades, and more" }}>
  ...
  * **Enterprise Grid support:** The [Slack agent](/docs/agent/slack) now installs on Slack Enterprise Grid and org-wide workspaces.
  ...
  * Automation stale review reminders: [Automations](/docs/automations) that open pull requests now send Slack reminders when reviews go stale.

  ## Bug fixes

  * Initial file and navigation load in the [web editor](/docs/editor) is faster.
...
<Update label="May 29, 2026" tags={["New releases","Improvements","Bug fixes"]} rss={{ title:"Editor and agent upgrades, new automations, API playground callbacks, and more" }}>
  ## Editor
  ...
  * **Searches Mintlify docs:** The agent can now search the Mintlify documentation while you work in the editor, so you're always connected to the source of truth for Mintlify while updating content.
  ...
  * **Draft improvements from user feedback:** This new automation reviews recent [page feedback](/docs/optimize/feedback) and opens pull requests based on what users report.
  ...
  * **AI discoverability for partially authenticated sites:** Deployments with [partial authentication](/docs/deploy/authentication-setup) now serve `sitemap.xml`, `llms.txt`, `llms-full.txt`, and `robots.txt` publicly, so AI crawlers and agents can discover your content even when parts of your site require authentication.
  ...
  * Snippet variables in code blocks: [Reusable snippets](/docs/create/reusable-snippets) now support variables inside code blocks, so you can parameterize examples consistently.

  ## Bug fixes

  * Custom heading IDs now preserve casing (camelCase, snake\_case, kebab-case).
...
<Update label="May 22, 2026" tags={["New releases","Improvements"]} rss={{ title:"Web editor enhancements, new automation, search improvements, and more" }}>
  ...
  * New **Draft improvements from assistant conversations** automation reviews trends from user questions and suggests documentation updates to address gaps. See [Automations](/docs/automations/reference#draft-improvements-from-assistant-conversations).
...
<Update label="May 8, 2026" tags={["New releases","Improvements"]} rss={{ title:"Expanded MCP access, assistant credit packages, AsyncAPI nested groups, and more" }}>
  ...
  Authenticated [MCP servers](/docs/ai/model-context-protocol) now return more of your content to authorized users. Authenticated users can search public pages plus any pages they have permission to access based on their user groups, instead of being limited to public pages only. [Client credentials](/docs/ai/model-context-protocol#client-credentials) return all public pages and authenticated pages that aren't restricted to specific groups.

  The Mintlify MCP `list_nodes` tool now supports filtering by `parentId` (with optional `recursive` traversal), multiple node types, and additional division scopes including `dropdown`, `anchor`, `product`, and `item`. Results are paginated through an opaque cursor. See the [Mintlify MCP](/docs/ai/mintlify-mcp) documentation for more details.

  ## Assistant credit packages

  The [assistant](/docs/assistant) billing experience has been updated to use credit-based packages across all plans, with consistent terminology in the dashboard and usage controls. No changes to existing credit balances or usage.

  ## AsyncAPI nested groups

  The `asyncapi` property now supports [nested groups](/docs/api-playground/asyncapi-setup#examples-with-nested-groups), so you can generate channel pages inside a subsection of a broader API group or combine multiple AsyncAPI specifications under a shared parent.

  ## Improvements

  * [Slack agent](/docs/agent/slack) now requests permission to run third-party actions such as sending an email or creating a ticket.
  * The [editor](/docs/editor) now lets you drag images and videos from the navigation tree directly onto a page to embed existing media inline.
  * Mermaid diagrams in the editor now support a fullscreen mode for easier editing of complex diagrams.
  * The custom domain dashboard now displays both verification `TXT` records (`_acme-challenge` and `_cf-custom-hostname`) with live verification status, so you can confirm DNS is correct before pointing your `CNAME` at Mintlify. See [custom domain](/docs/customize/custom-domain) for more details.
  * TLS certificate provisioning for [custom domains](/docs/customize/custom-domain#automatic-tls-provisioning) is now handled directly by Mintlify.
  * GitLab-connected deployments can now revalidate their Git source from the [Git settings](https://app.mintlify.com/settings/deployment/git-settings) page by clicking the **Active** badge, useful when branch options or configuration appear stale.
  * The [feedback dashboard](/docs/optimize/feedback) detailed feedback view now only shows submissions that include written comments, making qualitative feedback easier to scan.
</Update>
...
<Update label="May 1, 2026" tags={["New releases","Improvements"]} rss={{ title:"Revamped automations and editor, agent score, new Mintlify MCP, and more" }}>
  ...
  * **Custom automations**: In addition to built-in automations, you can create your own automations.
  ...
  See the [automations](/docs/automations) documentation for more details.
  ...
  See the [editor](/docs/editor) documentation for more details.

  The classic editor sunsets in May 2026.

  ## Agent score

  [Agent score](http://mintlify.com/score) evaluates how agent-ready your site is. It is a free tool built on the open source [AFDocs spec](https://afdocs.dev) with additional checks based on Mintlify's research and best practices.
  ...
  See the [Mintlify MCP](/docs/ai/mintlify-mcp) documentation for more details.

  ## Agent connections

  Ask the agent in [Slack](/docs/agent/slack) to set up any third-party integration that it should use as context. Prompt the agent to connect to a service like GitHub, Jira, Notion, Google Drive, Confluence, or any other service that you use.
  ...
  Search indexing now supports section-level boosts and page-level boosts, letting you promote specific content so it ranks higher for relevant queries. See the [Search](/docs/optimize/search#boost-search-ranking) documentation for more details.

  ## Improvements

  * Control whether OpenAPI-generated reference pages are included in your [Markdown export](/docs/ai/llmstxt) and `llms-full.txt`
  ...
  * The [Slack agent](/docs/agent/slack) can now clone any repository the Mintlify GitHub App has access to and is aware of every repo in your org when answering questions.
  ...
  * Fixed prose styling for the [Tabs](/docs/components/tabs) component.
  * Fixed offset alignment for blocks placed inside a [Column](/docs/components/columns).
...
<Update label="April 24, 2026" tags={["New releases","Improvements"]} rss={{ title:"Preview Deployment API, Git onboarding redesign, web editor improvements, and more" }}>
  ...
  A public API for generating preview deployments is now available. Use it to trigger previews from your own tooling—useful when your Git workflow doesn't map cleanly to standard branch-based preview triggers. See [Trigger preview deployment](/docs/api/preview/trigger) for more details.
  ...
  * **Accordion reordering**: Drag and drop to rearrange accordions inside an [accordion group](/docs/components/accordions) without editing MDX.
  * **Editable color component**: Update [color](/docs/components/color) swatches directly in the editor without editing MDX.
  * **SVG file upload**: Upload SVG files from the editor configuration view.

  ## Redirect destination checks

  The [mint broken-links](/docs/cli/commands#mint-broken-links) command now supports an option to validate redirect destinations. Catch redirects that point to missing pages before they reach production.
...
<Update label="April 17, 2026" tags={["New releases","Improvements"]} rss={{ title:"Visibility component, MCP filesystem access, agent feedback, Git setup wizard, and more" }}>
  ...
  The new [`Visibility`](/docs/components/visibility) component lets you show different content to human readers and AI agents on the same page. Wrap content in `<Visibility for="humans">` to display it only on the web version of a page or `<Visibility for="agents">` to include it only in [Markdown output](/docs/ai/markdown-export) consumed by AI tools. Useful for tailoring instructions, examples, or formatting to each audience without maintaining separate pages.

  ## Agent feedback

  Enable [agent feedback](/docs/optimize/feedback#agent-feedback) in your deployment settings to let agents submit structured feedback about your documentation. When turned on, Markdown versions of your pages include feedback instructions so agents know how to report any issues they encounter accessing your documentation. Available as a beta feature in the dashboard under deployment add-ons.

  ## MCP filesystem access

  [MCP servers](/docs/ai/model-context-protocol) now include a query docs filesystem tool that lets agents browse and read your documentation's file structure directly. Agents can navigate directories, read full page content, and batch-read multiple pages in a single call. This tool replaces the `get_page` tool.
  ...
  * The editor now resolves [`$ref` references](/docs/organize/settings#split-configuration-with-%24ref) in split configuration files.
  * The [Slack agent](/docs/agent/slack) system prompt has been improved for more accurate and helpful responses.
...
<Update label="April 10, 2026" tags={["New releases","Improvements"]} rss={{ title:"CLI analytics, directory listings, multi-skill support, Slack bot improvements, and more" }}>
  ...
  * `mint analytics conversation list` — recent assistant conversations
  ...
  Navigation groups with a [root page](/docs/organize/navigation#root-page) can now display a [directory listing](/docs/organize/navigation#directory-listings) of their child pages. Set the `directory` property on a group, tab, or the top-level `navigation` object to `"accordion"` or `"card"` to automatically generate an overview of the section's content. Useful for help centers and other sites where users need a quick way to browse all available pages in a section.

  ## Multi-skill support

  Serve multiple [skill files](/docs/ai/skillmd) from a `.mintlify/skills/` directory. Each subdirectory with a `SKILL.md` file is discovered and served automatically, so you can organize skills by feature or domain.

  ## Slack agent improvements

  * **Multiple deployments**: Organizations with multiple documentation sites can now use the [Slack agent](/docs/agent/slack) across all of them. The agent prompts you to choose a deployment on first use, and your preference is saved per Slack account.
  ...
  * [Automations](/docs/automations) now surface in the dashboard inbox for easier tracking. The automation list also displays PR names for quicker identification.
...
<Update label="April 3, 2026" tags={["New releases","Improvements"]} rss={{ title:"get_page MCP tool, password-protected previews, SAML group role mappings, and more" }}>
  ...
  [MCP servers](/docs/ai/model-context-protocol) that Mintlify generates now include a `get_page` tool that lets AI agents fetch full page content by path. Agents can retrieve complete documentation pages for deeper context beyond search results.

  ## Password-protected preview links

  [Preview deployments](/docs/deploy/preview-deployments) now support password-protection. Share preview links without exposing content publicly.

  ## SAML group role mappings

  Organizations using [SAML SSO](/docs/dashboard/sso) can now map SAML groups to dashboard roles for automatic role assignment when team members sign in.
  ...
  * Video support in [live preview](/docs/editor/live-preview). Embedded mp4, webm, and mov files now render correctly during local development.
  * [Automations](/docs/automations) can now be disabled and re-enabled from the dashboard.
  ...
  * Fixed [automation](/docs/automations) automerge toggle not properly disabling when set to off.
...
<Update label="March 27, 2026" tags={["New releases","Improvements"]} rss={{ title:"Linear and Notion integrations, offline exports, inline AI editing, and more" }}>
  ...
  Connect your Notion workspace from the agent settings page to give the [agent](/docs/agent) read access to your Notion content. The agent can reference Notion pages as context when generating documentation updates.

  ## Offline export

  Package your entire documentation site into a self-contained zip archive with the [`mint export`](/docs/deploy/export) CLI command. Share the zip so your users can view your docs in their browser without an internet connection or a Mintlify account.
  ...
  Split large configurations into smaller files using [`$ref` references](/docs/organize/settings#split-configuration-with-%24ref). Each `$ref` points to a separate JSON file resolved at build time.

  ## `data-active` CSS selectors

  Sidebar items, table of contents entries, and navigation dropdowns now expose a stable [`data-active` attribute](/docs/customize/custom-scripts#css-selectors) for custom CSS targeting. Use selectors like `.toc-item[data-active]` or `#sidebar-content li[data-active]` to style active states without relying on internal class names.

  ## Agent-skills discovery endpoint

  Documentation sites now serve a `/.well-known/agent-skills/` discovery endpoint following the [agent-skills 0.2.0 spec](https://schemas.agentskills.io/discovery/0.2.0/schema.json). AI agents can programmatically discover and fetch your [skill files](/docs/ai/skillmd#discovery) with content integrity verification.

  ## API playground credentials

  A new [`api.playground.credentials`](/docs/api-playground/overview) option sends browser-managed credentials (like cookies and session tokens) with API playground requests when the proxy is disabled. Useful for APIs that use cookie-based authentication.

  ## Improvements

  * [Custom heading IDs](/docs/create/text#custom-heading-ids) now supported. Search results deep-link to sections that use custom IDs.
  ...
  * [Heap analytics](/docs/integrations/analytics/heap) now properly forwards page views and custom events.
...
<Update label="March 20, 2026" tags={["New releases","Improvements"]} rss={{ title:"Authenticated MCP route, contextual menu expansions, self-hosted GitLab UI, and more" }}>
  ...
  A new authenticated route is available for [MCP](/docs/ai/model-context-protocol) servers, enabling secure search over documentation sites with authentication enabled.

  ## New contextual menu options

  The [contextual menu](/docs/ai/contextual-menu) now includes options for **Google AI Studio**, **Devin**, and **Windsurf**. Users can open any documentation page directly in these tools with the page content preloaded as context.

  ## Self-hosted GitLab configuration

  You can now configure [self-hosted GitLab](/docs/deploy/gitlab) instances directly from the dashboard. GitLab API requests are proxied through the server, removing the need for browser-based authentication to your GitLab instance.
...
<Update label="March 13, 2026" tags={["New releases","Improvements"]} rss={{ title:"Automation templates and CLI command, Agent API v2, and more" }}>
  ...
  Create [automations](/docs/automations) faster with pre-built templates in the [dashboard](https://app.mintlify.com/products/automations). Choose from templates for changelog generation, API docs sync, feature documentation drafts, broken link detection, SEO audits, and more. Templates include optimized prompts and trigger configurations to get you started quickly.

  ## Automation CLI command

  Create [automations](/docs/automations) files from the command line with the `mint automation` CLI command. Interactive prompts guide you through name, trigger, context, and instructions.

  ## Agent API v2

  Trigger the agent programmatically with the new `/v2/agent` [endpoints](/docs/api/agent/v2/create-agent-job). Submit a job with a prompt and optional context, then poll for completion status and the resulting pull request link. Useful for integrating agent-driven documentation updates into your own CI/CD pipelines or internal tooling.
  ...
  View feedback broken down by individual pages in your [analytics](/docs/optimize/analytics) dashboard.
...
<Update label="March 6, 2026" tags={["New releases","Improvements"]} rss={{ title:"Auto-generate documentation from public GitHub repositories, assistant multimodality, and more improvements" }}>
  ...
  [Auto-generate a complete documentation site](https://mintlify.com/explore) from any public GitHub repository. Replace `github.com` with `mintlify.com` in any public repository URL to instantly preview generated documentation. If you are the repository owner, claim the documentation site to add it to your Mintlify organization and customize it.

  ## Assistant improvements

  The [assistant](/docs/assistant/index) now supports multi-modal input. Attach files to messages to provide additional context. Drag and drop or paste images, PDFs, and code files directly into the chat.

  The assistant can now generate page-specific [example questions](/docs/assistant/configure#add-sample-questions).
  ...
  The default [Vale configuration](/docs/deploy/ci#configuration) now parses MDX as MD to avoid fragility with JSX. Previously, Vale returned false positive errors on any JSX inside of MDX components. Now, Vale parses MDX as MD to avoid these errors with tokenIgnores and blockIgnores for MDX and JSX elements that Vale should skip.

  ## Improvements

  * Automations now support up to 50 runs per day, increased from 20.
  * [Full base URL](/docs/organize/settings-api) now supported in the API playground by configuring `api.url` in `docs.json`.
...
<Update label="February 27, 2026" tags={["New releases","Improvements"]} rss={{ title:"Automate documentation tasks with automations and collaborate in the editor with comments and suggestions" }}>
  ## Automations

  Automate documentation tasks with [automations](/docs/automations). Set up scheduled or event-triggered automations that run the agent to update your documentation.
  ...
  New [Luma theme](/docs/customize/themes) available for a minimal, calm design.
  ...
  * OpenAPI and AsyncAPI specs now included in [llms.txt](/docs/ai/llmstxt).
  * Adobe Analytics integration support.
  * Model Context Protocol (MCP) now works with [partially authenticated docs](/docs/ai/model-context-protocol).
...
<Update label="February 20, 2026" tags={["New releases","Improvements"]} rss={{ title:"Agent configuration files and custom frontend" }}>
  ...
  Use `.mintlify/Assistant.md` and `.mintlify/AGENTS.md` files in your repository to customize the agent and assistant behaviors. See [Customize assistant behavior](/docs/assistant/customize) and [Customize agent behavior](/docs/agent/customize) for more information.

  ## Custom frontend

  Bring your own frontend and use the Mintlify content engine to publish your documentation and enable AI tools. See the [custom frontend guide](/docs/guides/custom-frontend) for more information.
...
<Update label="February 13, 2026" tags={["New releases","Improvements"]} rss={{ title:"Prompt component, Sequoia theme, Tabler icons, and editor improvements" }}>
  ...
  Use the [prompt](/docs/components/prompt) component to display copyable prompts. Designed for sharing AI prompts and supports opening prompts directly in Cursor.

  ## Sequoia theme

  The [Sequoia theme](/docs/customize/themes) features redesigned table of contents with the [contextual menu](/docs/ai/contextual-menu) in the sidebar.
  ...
  The [contextual menu](/docs/ai/contextual-menu) now includes an option to add a copyable command to install your docs MCP server.

  ## `root` in groups

  Navigation groups now support a [`root`](/docs/organize/navigation#root-page) property to set a landing page for the group. Clicking the group header navigates to the root page instead of expanding the group.
...
<Update label="January 30, 2026" tags={["New releases","Improvements"]} rss={{ title:"Simplified skills installation, revamped analytics, editor improvements, and more" }}>
  ...
  Install [skill.md files](/docs/ai/skillmd) using just a documentation URL. No path to the `skill.md` file needed.
  ...
  * Stability improvements
  ...
  Manage all [assistant features](/docs/assistant/index) in one page with a cleaner layout and better UX.
  ...
  * "Open in Grok" support in the [contextual menu](/docs/ai/contextual-menu)
...
<Update label="January 23, 2026" tags={["New releases","Improvements"]} rss={{ title:"skills.md, audit logs, and markdown export" }}>
  ...
  Documentation sites now auto-generate a `skill.md` file based on your API references, guides, and features to give AI agents a comprehensive skills file with context on how to help people use your product. `skill.md` replaces the `install.md` approach with more structured capabilities and wider adoption. If you have a custom `skills.md` file at the root of your documentation repository, it replaces the auto-generated version. See [skill.md](/docs/ai/skillmd) for more information.

  ## Audit logs for authentication events

  Events are now tracked in [audit logs](/docs/dashboard/audit-logs) for visibility and compliance.
  ...
  * Combined authentication, partial authentication, and personalization into a single authentication experience. See [Authentication setup](/docs/deploy/authentication-setup) for more information.
...
<Update label="January 9, 2026" tags={["New releases","Improvements"]} rss={{ title:"Implicit snippets and assistant conversation insights for all" }}>
  ...
  Snippets now work without explicit configuration. Previously, you needed to define snippets in a specific `/snippets` directory. Now, you can import any file as a snippet from anywhere in your docs for more flexible content reuse. See [Reusable snippets](/docs/create/reusable-snippets) for more information.
  ...
  * **Mermaid controls**: Interactive controls for [Mermaid diagrams](/docs/components/mermaid-diagrams) to navigate large diagrams.
...
<Update label="December 23, 2025" tags={["New releases","Improvements"]} rss={{ title:"Web editor improvements and assistant insights" }}>
  ...
  The [web editor](/docs/editor/index) now has a more intuitive interface and new capabilities.
  ...
  * **Improved stability**: Stability improvements and mobile responsiveness.
  ...
  The [agent](/docs/agent) can now suggest documentation updates based on assistant conversations. When enabled, the agent analyzes questions users ask the assistant to identify patterns that indicate missing or unclear documentation.
  ...
  You can now [customize your Slack bot](/docs/ai/slack-bot) directly from the dashboard.
...
<Update label="December 19, 2025" tags={["Improvements"]} rss={{ title:"Agent, assistant, and editor improvements" }}>
  ...
  * Added support for reading [custom instructions](/docs/agent/customize) from an `AGENTS.md` file.
  ...
  * Added [shareable preview links](/docs/editor/branching-and-publishing#access-and-share-a-preview) from the editor toolbar.
  * Updated [publishing workflow](/docs/editor/branching-and-publishing) to support one-click publishing or creating pull requests.
  ...
  * [Relative imports](/docs/create/reusable-snippets#import-snippets-into-pages) are now supported for snippets.
...
<Update label="December 12, 2025" tags={["New releases","Improvements"]} rss={{ title:"Agent suggestions and Q&A bots" }}>
  ...
  Add a bot powered by the assistant to your [Slack workspaces](/docs/ai/slack-bot). The bot answers questions in real-time based on your documentation, so it provides accurate, up-to-date answers to your community.
...
<Update label="December 8, 2025" tags={["New releases"]} rss={{ title:"Last modified dates",description:"Show when pages were last updated with automatic timestamps" }}>
  ...
  You can now display "last modified" timestamps on all pages in your documentation. Enable this feature by setting the [`timestamp`](/docs/organize/settings-seo#metadata) flag in your `docs.json` settings.
...
<Update label="December 3, 2025" tags={["New releases"]} rss={{ title:"Assistant query bucketing",description:"Automatically group similar assistant queries to identify common user questions" }}>
  ...
  [Assistant insights](https://app.mintlify.com/products/assistant) now automatically groups similar queries together into question categories, which makes it easier to identify patterns in what your users are asking about.
...
<Update label="December 1, 2025" tags={["New releases"]} rss={{ title:".mintignore support",description:"Exclude files and directories from your published documentation" }}>
  ...
  * Excluded files don't appear in published docs, aren't indexed for search, and aren't accessible to visitors.
  ...
  Learn more in the [.mintignore documentation](/docs/organize/mintignore).
</Update>
...
<Update label="September 13, 2025" tags={["New releases","Improvements"]} rss={{ title:"Weekly Updates",description:"AI 404 suggestions, assistant web search with external sources, security enhancement, and reliability fixes" }}>
  ...
  * Fixed a bug where MCP server names concatenated with tool calls were sometimes exceeding the 60 character length MCP clients enforce
...
<Update label="August 16, 2025" tags={["Improvements"]} rss={{ title:"Weekly Updates",description:"Performance optimizations, LLM integrations, web editor polish, and API playground enhancements" }}>
  ...
  * We added a feature such that you can now [specify API pages using just your docs.json](/docs/api-playground/openapi-setup#auto-populate-api-pages) without creating any new mdx files.
  * Support for [`webhook pages`](/docs/api-playground/openapi-setup#webhooks) in docs navigation from OpenAPI specs
  ...
  * Added [Hightouch analytics integration](/docs/integrations/analytics/hightouch)
  ...
  * Added CSS selectors to [breadcrumbs](/docs/organize/navigation#breadcrumbs) for custom CSS targeting
...
<Update label="July 2025" tags={["New releases","Improvements"]} rss={{ title:"July Product Updates",description:"Slack app integration, hosted MCP servers, Cursor integration, and performance improvements" }}>
  ...
  * Deflect support interruptions: Turn daily questions into instant, self-serve answers
  ...
  Learn more in our [Slack bot guide](/docs/ai/slack-bot).

  ## Hosted MCP servers

  Deploy hosted Model Context Protocol (MCP) servers directly through Mintlify to integrate with AI tools like Claude, Cursor, and others. Learn more in our [MCP guide](/docs/ai/model-context-protocol).

  Help users quickly connect your MCP server to Cursor or VS Code from any page in your docs via the contextual menu. See [Contextual menu](/docs/ai/contextual-menu) for more information.
...
<Update label="June 2025" tags={["New releases","Improvements"]} rss={{ title:"June Product Updates",description:"AI assistant updates and subscribable changelogs" }}>
  ...
  * "Ask AI" shortcut on code blocks in documentation to generate explanations from the assistant
  ...
  Learn more in the [assistant docs](/docs/assistant/index).
  ...
  * Integrate RSS-enabled updates with Slack, email, and other tools
  ...
  Learn more in our new [Changelog guide](/docs/create/changelogs)
</Update>
...
<Update label="May 2025" tags={["Improvements"]}>
  ...
  * Upload multipart or form data files
  ...
  Learn more at [API playground docs.](/docs/api-playground/)
...
<Update label="April 2025" tags={["New releases","Improvements"]}>
  ...
  <Frame>
    <img src="https://mintcdn.com/mintlify/TcGu3ccxDpo8vh9U/images/webeditor3.jpeg?fit=max&auto=format&n=TcGu3ccxDpo8vh9U&q=85&s=1a0e1ce1db4d3b6bf7832ee20406ce8b" alt="Webeditor3 Jpe" width="3240" height="3240" data-path="images/webeditor3.jpeg" />
  </Frame>
  ...
  * Fixed block quotes creating more block quotes when trying to deselect
  ...
  <Frame>
    <img src="https://mintcdn.com/mintlify/WXXCCJWDplNJgTwZ/images/changelog/translations.png?fit=max&auto=format&n=WXXCCJWDplNJgTwZ&q=85&s=655f5c32e66f402026a54881442b8305" alt="AI Translations graphic" width="995" height="611" data-path="images/changelog/translations.png" />
  </Frame>
...
<Update label="March 2025" tags={["New releases","Improvements"]}>
  ...
  <Frame>
    <img src="https://mintcdn.com/mintlify/WXXCCJWDplNJgTwZ/images/changelog/mcpgenerator.png?fit=max&auto=format&n=WXXCCJWDplNJgTwZ&q=85&s=9a6ad291fbcaaf5fafc5a5a54a6b020b" alt="screenshot of MCP server generator" width="1200" height="675" data-path="images/changelog/mcpgenerator.png" />
  </Frame>

  Generate MCP servers so that AI applications can interact with your docs or APIs. Written content is automatically generated as an MCP server, and you can generate an MCP server from your OpenAPI spec with one click. Check out [docs on getting started with MCP.](/docs/ai/model-context-protocol)
...
<Update label="February 2025" tags={["New releases","Improvements"]}>
  ...
  <Frame>
    <img src="https://mintcdn.com/mintlify/WXXCCJWDplNJgTwZ/images/changelog/docsjson.png?fit=max&auto=format&n=WXXCCJWDplNJgTwZ&q=85&s=63fc86877606ea18103ea7eb0a287dc3" alt="docs.json screenshot" width="1200" height="630" data-path="images/changelog/docsjson.png" />
  </Frame>
  ...
  <Frame>
    <img src="https://mintcdn.com/mintlify/WXXCCJWDplNJgTwZ/images/changelog/themes.png?fit=max&auto=format&n=WXXCCJWDplNJgTwZ&q=85&s=b27f24b3138064d256eb5d4bf151de4a" alt="graphic with text &#x22;Themes v2&#x22;" width="1200" height="630" data-path="images/changelog/themes.png" />
  </Frame>
...
<Update label="January 2025" tags={["Improvements"]}>
  ...
  * Improved `docs.json` file editing
  ...
  We’ve overhauled the design and performance of the [API Playground](/docs/api-playground/). Updates include:
...
<Update label="December 2024">
  ...
  <Frame>
    <img src="https://mintcdn.com/mintlify/WXXCCJWDplNJgTwZ/images/changelog/authentication.png?fit=max&auto=format&n=WXXCCJWDplNJgTwZ&q=85&s=8ccc5ffdd641bcae823e1fe38b47301f" alt="Authentication screenshot" width="1200" height="630" data-path="images/changelog/authentication.png" />
  </Frame>
...
<Update label="November 2024">
  ...
  <Frame>
    <img src="https://mintcdn.com/mintlify/WXXCCJWDplNJgTwZ/images/changelog/ai-assistant.jpg?fit=max&auto=format&n=WXXCCJWDplNJgTwZ&q=85&s=5feefba79cec6b665ee88ac2f80f5b25" alt="AI Assistant" width="1280" height="800" data-path="images/changelog/ai-assistant.jpg" />
  </Frame>
  ...
  We've improved our support for syncing with GitLab, such as enabling automated updates and preview deployments. Check out our [docs on GitLab](/docs/deploy/gitlab) to get started.

  ## Web Editor

  <Frame>
    <img src="https://mintcdn.com/mintlify/WXXCCJWDplNJgTwZ/images/changelog/webeditor.jpg?fit=max&auto=format&n=WXXCCJWDplNJgTwZ&q=85&s=26ea32fc35ac8f432d4a3861fc2c4bba" alt="Web Editor" width="5760" height="3520" data-path="images/changelog/webeditor.jpg" />
  </Frame>
  ...
  Check out our [docs on getting started with Web Editor](/docs/editor).

  ## /llms.txt support

  <Frame>
    <img src="https://mintcdn.com/mintlify/WXXCCJWDplNJgTwZ/images/changelog/llms.jpg?fit=max&auto=format&n=WXXCCJWDplNJgTwZ&q=85&s=60828382c88fd24d464b5c2cae759760" alt="llms.txt support" width="1200" height="630" data-path="images/changelog/llms.jpg" />
  </Frame>
...
<Update label="October 2024">
  ...
  Launched a new [Update component](/docs/components/update) to make it easier to display and report updates (like this one) to your users.

  <Frame>
    <img src="https://mintcdn.com/mintlify/WXXCCJWDplNJgTwZ/images/changelog/changelog.jpg?fit=max&auto=format&n=WXXCCJWDplNJgTwZ&q=85&s=94d2c26b046b4e3dd2dae543021ba410" alt="Changelog" width="4800" height="2520" data-path="images/changelog/changelog.jpg" />
  </Frame>
  ...
  ```javascript Line Highlighting Example highlight={1,3-5} theme={null}
  ...
  <Frame>
    <img src="https://mintcdn.com/mintlify/WXXCCJWDplNJgTwZ/images/changelog/advanced-footer.gif?s=37230f2a6cad69fdcbcba0352db56ffd" alt="Advanced Footer" width="1040" height="720" data-path="images/changelog/advanced-footer.gif" />
  </Frame>
...
<Update label="September 2024">
  ...
  <Frame>
    <img src="https://mintcdn.com/mintlify/WXXCCJWDplNJgTwZ/images/changelog/custom-fonts.jpeg?fit=max&auto=format&n=WXXCCJWDplNJgTwZ&q=85&s=dea37048248a6a332803123b30893d7c" alt="Custom Fonts" width="2048" height="1075" data-path="images/changelog/custom-fonts.jpeg" />
  </Frame>
  ...
  Add an `img` property to a card to display an image on the top of the card. Learn more about it [here](/docs/components/cards#image-cards).

  ## Update Speed Performances

  <Frame>
    <img src="https://mintcdn.com/mintlify/WXXCCJWDplNJgTwZ/images/changelog/performance-improvements.png?fit=max&auto=format&n=WXXCCJWDplNJgTwZ&q=85&s=420764afe13643dd0641398671be2c4e" alt="Performance Improvements" width="1800" height="945" data-path="images/changelog/performance-improvements.png" />
  </Frame>
  ...
  <Frame>
    <img src="https://mintcdn.com/mintlify/WXXCCJWDplNJgTwZ/images/changelog/seo-improvements.jpeg?fit=max&auto=format&n=WXXCCJWDplNJgTwZ&q=85&s=e7e647e7cde063a0ff9021a2ca8533e7" alt="SEO Improvements" width="2048" height="1075" data-path="images/changelog/seo-improvements.jpeg" />
  </Frame>
  ...
  * **Troubleshooting for API pages**: API pages could be complicated so we listed common issues to help you sort them out quickly — [Read the docs](/docs/api-playground/troubleshooting)
</Update>
...
<Update label="July 2024">
  ...
  <Frame>
    <img src="https://mintcdn.com/mintlify/WXXCCJWDplNJgTwZ/images/changelog/widget.png?fit=max&auto=format&n=WXXCCJWDplNJgTwZ&q=85&s=09c9ba11ff9770523085eeda1d1e4f9d" alt="AI Widget" width="1548" height="1021" data-path="images/changelog/widget.png" />
  </Frame>
